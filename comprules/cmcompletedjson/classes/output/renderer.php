...
namespace comprules_cmcompletedjson\output;

defined('MOODLE_INTERNAL') || die();

use comprules_cmcompleted\cmcompletedjson_comprule;
use stdClass;
...
class renderer extends \local_coursegoals\output\renderer
{
    public function renderTaskDetails($task)
    {
        ...
        $params = cmcompletedjson_comprule::decodeParams($task->comprule_params);
        ...
        if ($cminfo) {
            ...
            return $this->render_from_template('comprules_cmcompletedjson/task_details', $data);
        } else {
    ...
    public function renderCompletion($task, $taskrecord) {
        ...
        switch ($taskrecord->completed) {
            case cmcompletedjson_comprule::COMPLETION_EMPTY:
                $pix_id = 'cb-empty';
                break;
            case cmcompletedjson_comprule::COMPLETION_DONE:
                $pix_id = 'cb-complete';
                break;
            case cmcompletedjson_comprule::COMPLETION_WARNING:
                $pix_id = 'cb-warning';
                break;
            case cmcompletedjson_comprule::COMPLETION_FAIL:
                $pix_id = 'cb-fail';
                break;
        }
