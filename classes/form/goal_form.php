...
use local_coursegoals\api_dforms;
...
class goal_form extends \core_form\dynamic_form {
    ...
    protected function definition()
    {
        ...
        if ($action == Goal::ACTION_CREATE || $action == Goal::ACTION_EDIT) {
            ...
            $mform->addElement('static', 'confirm', '', get_string('ays_delete_goal', 'local_coursegoals'));
    ...
    public function process_dynamic_submission() {
        $data = $this->get_data();
        if ($data->action == Goal::ACTION_CREATE) {
            list($result, $errors, $redirecturl) = api_dforms::createGoal($data);
        } else if ($data->action == Goal::ACTION_EDIT) {
            list($result, $errors, $redirecturl) = api_dforms::editGoal($data);
        } else if ($data->action == Goal::ACTION_DELETE) {
            list($result, $errors, $redirecturl) = api_dforms::deleteGoal($data);
        }
        return [
            'result' => $result,
            'errors' => $errors,
    ...
    protected function get_context_for_dynamic_submission(): context {
        // TODO: check if context pulls properly
